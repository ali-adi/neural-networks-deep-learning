...
class TIMNET_Model(Common_Model):
    ...
    def train(self, x, y):
        ...
        for train, test in kfold.split(x, y):
            ...
            # Change extension to .weights.h5 as required when save_weights_only=True
            weight_path = folder_address + '/' + str(self.args.split_fold) + "-fold_weights_best_" + str(i) + ".weights.h5"
            checkpoint = callbacks.ModelCheckpoint(
                weight_path, verbose=1, save_weights_only=True, save_best_only=False
            )
            h = self.model.fit(
                x[train], y_train,
        ...
        writer = pd.ExcelWriter(
            resultpath + self.args.data + '_' + str(self.args.split_fold) + 'fold_' +
            str(round(self.acc * 10000) / 100) + "_" + str(self.args.random_seed) + "_" + now_time + '.xlsx'
        )
    ...
    def test(self, x, y, path):
        ...
        for train, test in kfold.split(x, y):
            self.create_model()
            # Change extension to .weights.h5 in the test function as well
            weight_path = path + '/' + str(self.args.split_fold) + "-fold_weights_best_" + str(i) + ".weights.h5"
            self.model.fit(
                x[train], y[train],
                validation_data=(x[test], y[test]),
                batch_size=64, epochs=0, verbose=0
            )
            self.model.load_weights(weight_path)
            best_eva_list = self.model.evaluate(x[test], y[test])
            avg_loss += best_eva_list[0]
