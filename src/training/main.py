...
def main():
    ...
    parser.add_argument(
        "--data", type=str, default="EMODB", help="Dataset name: EMODB or RAVDE"
    )
    parser.add_argument(
        "--feature_type",
        type=str,
        default="MFCC",
        choices=["MFCC", "LOGMEL", "HUBERT"],
    ...
    parser.add_argument(
        "--filter_size", type=int, default=39, help="Number of filters."
    )
    parser.add_argument(
        "--dilation_size",
        type=int,
    ...
    parser.add_argument(
        "--kernel_size", type=int, default=2, help="Kernel size for convolution layers."
    )
    parser.add_argument(
        "--stack_size", type=int, default=1, help="Number of stacked temporal blocks."
    )
    parser.add_argument(
        "--split_fold", type=int, default=2, help="Cross-validation fold count."
    )
    ...
    if args.feature_type == "MFCC":
        ...
        data_path = os.path.join(
            "datas", "features", "MFCC", subfolder, f"{args.data}.npy"
        )
    ...
    cuda_gpus = (
        tf.config.list_physical_devices("GPU") if tf.test.is_built_with_cuda() else []
    )
